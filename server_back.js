...
app.get('/api/rtm/discover', async (req, res) => {
  const type = req.query.type === 'tv' ? 'tv' : 'movie';
  const genre = req.query.genre;
  const page = req.query.page || 1;
  const ck = `discover:${type}:${genre}:${page}`;

  let data = cGet(ck);
  if (!data) {
    try {
      const url = `${C.mb}/discover/${type}?api_key=${C.mk}&with_genres=${genre}&page=${page}&language=fr-FR&sort_by=popularity.desc`;
      const r = await axGet(url);
      data = (r.data.results || []).map(m => ({
        ...m,
        streamToken: btoa(JSON.stringify({ type, id: m.id }))
      }));
      cSet(ck, data);
    } catch (e) {
      return res.json({ results: [] });
